const NavBtn = ({ toggleDrawer, state }) => {
  const genericHamburgerLine = `h-1 my-1 rounded-full ${state ? 'bg-sky-600' : 'bg-[#77FFD6]'} transition ease-in-out transform duration-300`;
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`${genericHamburgerLine} ${
            state ? "opacity-100 w-full rotate-45" : "opacity-100 group-hover:opacity-80 w-full"
          }`}
        />
        <div
          className={`${genericHamburgerLine} ${
